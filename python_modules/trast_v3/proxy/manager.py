"""Enhanced proxy manager with circuit breaker and fallback strategies for Trast Parser V3"""
...
from utils.exceptions import ProxyValidationError, ProxyConnectionError, ProxyCircuitBreakerOpen
...
class ProxyManager:
    """Enhanced thread-safe proxy manager with circuit breaker and caching"""
    ...
    def __init__(self, country_filter: Optional[List[str]] = None, cache_dir: Path = None):
        ...
        # Circuit breakers for proxies (one per proxy)
    ...
    def _get_circuit_breaker(self, proxy: Dict) -> CircuitBreaker:
        """Get or create circuit breaker for proxy"""
        proxy_key = f"{proxy['ip']}:{proxy['port']}"
        
        with self.circuit_breaker_lock:
            if proxy_key not in self.circuit_breakers:
                self.circuit_breakers[proxy_key] = CircuitBreaker(
                    name=f"Proxy-{proxy_key}",
                    failure_threshold=5,
                    recovery_timeout=300
                )
            return self.circuit_breakers[proxy_key]
    ...
    def get_proxy_for_thread(self, thread_id: int) -> Optional[Dict]:
        """
        Get a proxy for a specific thread (thread-safe) with circuit breaker
        ...
        with self.lock:
            ...
            if thread_id in self.thread_proxies:
                proxy = self.thread_proxies[thread_id]
                # Check circuit breaker
                cb = self._get_circuit_breaker(proxy)
                if cb.get_state().value == "open":
                    # Circuit is open, get new proxy
                    logger.warning(f"Proxy {proxy['ip']}:{proxy['port']} circuit breaker is open")
    ...
    def mark_proxy_failed(self, proxy: Dict):
        """Mark proxy as failed (thread-safe) and update circuit breaker"""
        with self.lock:
            proxy_key = f"{proxy['ip']}:{proxy['port']}"
    ...
    def mark_proxy_successful(self, proxy: Dict, context: Optional[Dict] = None):
        """Mark proxy as successful (thread-safe) and update circuit breaker"""
        with self.lock:
            proxy_key = f"{proxy['ip']}:{proxy['port']}"
    ...
    def get_working_proxies_parallel(
        ...
        """
        Find working proxies in parallel with circuit breaker support
        ...
        def validate_proxy_worker(proxy: Dict):
            """Worker function to validate a proxy with circuit breaker"""
            proxy_key = f"{proxy['ip']}:{proxy['port']}"
            cb = self._get_circuit_breaker(proxy)
            
            # Check circuit breaker
            if cb.get_state().value == "open":
                logger.debug(f"Proxy {proxy_key} circuit breaker is open, skipping")
                return False
            
            try:
                # Use circuit breaker to call validation
                is_valid, context = cb.call(validate_proxy_for_trast, proxy)
                ...
                if is_valid:
                    ...
                    return False
            except ProxyCircuitBreakerOpen:
                logger.debug(f"Proxy {proxy_key} circuit breaker is open")
                return False
        ...
        with ThreadPoolExecutor(max_workers=PROXY_SEARCH_THREADS) as executor:
            ...
            # Start with successful proxies (not in circuit breaker open state)
            for proxy in self.successful_proxies[:count * 2]:  # Try more than needed
                if len(working_proxies) >= count:
                    break
                proxy_key = f"{proxy['ip']}:{proxy['port']}"
                if proxy_key not in self.failed_proxies:
