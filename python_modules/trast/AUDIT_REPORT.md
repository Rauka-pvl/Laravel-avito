...
### Проверка IP
- ⚠️ **ПРОБЛЕМА**: В `get_driver_with_working_proxy()` (строка 457-467) проверка IP через публичные сервисы может вызывать DNS проблемы, если прокси не настроен правильно. Сейчас используется `ifconfig.me`.
